...
# Mapping of utility and allocation functions
funcs = {
    'f1': model.f1,
    'f2': model.f2,
    'f3': model.f3,
...
# Compute and plot derivatives dU/dx and dh/dx vs x
for label, U_func in funcs.items():
    ...
    dU_dx = np.gradient(U_vals, x_vals)
    dh_dx = np.gradient(h_vals, x_vals)

    plt.figure()
    plt.plot(x_vals, dU_dx, label='dU/dx', color='tab:green')
    plt.plot(x_vals, dh_dx, label='dh/dx', color='tab:red')
    plt.xlabel('x')
    plt.ylabel('Derivative')
    plt.title(f'{label}: derivatives vs x')
    plt.legend()
    plt.tight_layout()

...
v_ranges = {
    ...
    'f5': (1, 15), 'f6': (1, 50), 'f7': (0.1, 10), 'f8': (20, 100),
}
...
for label, U_func in funcs.items():
    ...
    ax.plot(v_norm, dU_dh, color='green', label='dU/dh')
    ax.plot(v_norm, dU_dv, color='purple', label='dU/dv')
    ax.set_xlabel('Normalized v (0-1)')
    ax.set_ylabel('Derivatives')
    ax.set_title(f'{label}: Derivatives of utility vs normalized v')
    ax.legend()

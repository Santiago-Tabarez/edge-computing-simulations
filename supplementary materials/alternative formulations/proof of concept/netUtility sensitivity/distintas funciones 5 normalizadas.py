...
for label, U_func in funcs.items():
    ...
    u_line, = ax1.plot(v_norm, U_vals, color='tab:blue', label='utility U')
    h_line, = ax2.plot(v_norm, h_vals, color='tab:orange', label='allocation h')
    #ax2.set_yscale('log')

    ax1.set_xlabel('Normalized v (0-1)')
    ax1.set_ylabel('utility U')
    ax2.set_ylabel('allocation h )')

    lines = [u_line, h_line]
    labels = [line.get_label() for line in lines]
    ax1.legend(lines, labels, loc='upper left')

    ax1.set_title(f'{label}: utility and allocation vs normalized v')
    plt.tight_layout()

...
v_ranges = {
    'f1': (1, 200), 'f2': (1, 10), 'f3': (1, 6), 'f4': (1, 1000),
    'f5': (1, 15), 'f6': (1, 50), 'f7': (0.1, 10), 'f8': (2, 50),
}
...
for label, U_func in funcs.items():
    ...
    # Normalizar v_vals entre 0 y 1
    v_norm = (v_vals - v_min) / (v_max - v_min)

    h_vals = h_funcs[label](x_vals)
    U_vals = np.array([U_func(x) for x in x_vals])

    # Calcular derivadas numéricas
    dU_dh = np.gradient(U_vals, h_vals)
    dU_dv = np.gradient(U_vals, v_vals)

    fig, ax = plt.subplots(1, 2, figsize=(12, 5))

    ax[0].plot(v_norm, dU_dh, color='green', label='dU/dh')
    ax[0].set_xlabel('Normalized v (0-1)')
    ax[0].set_ylabel('dU/dh')
    ax[0].set_title(f'{label}: dU/dh vs normalized v')
    ax[0].legend()

    ax[1].plot(v_norm, dU_dv, color='purple', label='dU/dv')
    ax[1].set_xlabel('Normalized v (0-1)')
    ax[1].set_ylabel('dU/dv')
    ax[1].set_title(f'{label}: dU/dv vs normalized v')
    ax[1].legend()

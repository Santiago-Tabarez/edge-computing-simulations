...
# Plot allocation (h) and utility (U) together
for label, U_func in funcs.items():
    ...
    plt.figure()
    plt.plot(v_vals, h_vals, label='allocation h')
    plt.plot(v_vals, U_vals, label='utility U')
    plt.xlabel('v')
    plt.ylabel(label)
    plt.title(f'{label}: allocation and utility vs v')
    plt.legend()
    plt.tight_layout()

...
d = 1.0e-05      # marginal cost

# Range of beta (benefit factor)
# Threshold for Case0 and Case1 is beta > d/(l*mu) ~ 1.323e-06. Start slightly above.
beta_vals = np.linspace(1.5e-06, 1.5e-05, 300)
...
titles_U = {
    0: "Case 0: $U(h^*)$ vs $\\beta$",
    1: "Case 1: $U(h^*)$ vs $\\beta$",
    2: "Case 2: $U(h^*)$ vs $\\beta$",
    3: "Case 3: $U(h^*)$ vs $\\beta$"
}
...
for case in range(4):
    ...
    # Plot h*
    ax_h.plot(beta_vals, h_vals, color='tab:orange', linewidth=3)
    ax_h.set_title(titles_h[case], fontsize=14)
    ax_h.set_xlabel('$\\beta$ (Benefit factor)', fontsize=12)
    ax_h.set_ylabel('$h^*$ (millicores)', fontsize=12)
    ax_h.grid(False)
    ax_h.locator_params(axis='x', nbins=5)

    # Plot U
    ax_U.plot(beta_vals, U_vals, color='tab:blue', linewidth=3)
    ax_U.set_title(titles_U[case], fontsize=14)
    ax_U.set_xlabel('$\\beta$ (Benefit factor)', fontsize=12)
